...
const App: React.FC = () => {
    ...
    const parseUrlParams = useCallback(() => {
        try {
            ...
            if (targetView) {
                 if (['home', 'products', 'productDetail', 'ofertas', 'ia', 'catalog', 'about', 'contact', 'blog', 'blogPost', 'checkoutSummary', 'myOrders'].includes(targetView)) {
    ...
    // New: Buy Now / Google Play Logic (Add & Navigate directly to Checkout)
    ...
    return (
        <div className="flex flex-col min-h-screen bg-white font-sans text-gray-800 relative">
            ...
            {/* Floating WhatsApp Button - Z-INDEX 9999 TO BE VISIBLE ALWAYS */}
            <a 
                href="https://api.whatsapp.com/send?phone=34661202616&text=Hola,%20necesito%20ayuda%20con%20mi%20pedido."
                target="_blank"
                rel="noopener noreferrer"
                className="fixed bottom-24 md:bottom-8 right-6 z-[9999] bg-green-500 hover:bg-green-600 text-white p-4 rounded-full shadow-2xl transition-transform hover:scale-110 flex items-center justify-center gap-2 group border-4 border-white"
                aria-label="Chat de WhatsApp"
                title="Ayuda por WhatsApp"
            >
                <WhatsAppFloatIcon />
                <span className="hidden group-hover:inline-block font-bold whitespace-nowrap transition-all">661-202-616</span>
            </a>
