...
const CheckoutSummaryPage: React.FC<CheckoutSummaryPageProps> = ({ 
    ...
    const handleFinalizeOrder = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (result && result.id) {
                finalOrderNumber = result.id.toString();
            } else {
                // Fallback for demo purposes if API is blocked by browser
                finalOrderNumber = "VP-" + Math.floor(Math.random() * 1000000).toString();
            }
            setOrderNumber(finalOrderNumber);
            
            // --- SAVE TO LOCAL HISTORY (Simulate User Database) ---
            const newOrder = {
                id: finalOrderNumber,
                ...
                status: 'Verificado',
                items: cartItems,
                paymentMethod: paymentMethod === 'google_play' ? 'Google Play' : 'Tarjeta'
            };
            
            try {
                const existingOrders = JSON.parse(localStorage.getItem('vellaperfumeria_past_orders') || '[]');
                existingOrders.push(newOrder);
                localStorage.setItem('vellaperfumeria_past_orders', JSON.stringify(existingOrders));
                
                // Clear the cart
                localStorage.removeItem('vellaperfumeria_cart');
                // Note: We can't clear the cartItems prop directly here since it comes from parent,
                // but usually you would trigger a callback to clear parent state.
            } catch (err) {
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            <button 
                                ...
                                className="w-full bg-white text-black border-2 border-black font-bold py-4 rounded-xl hover:bg-gray-50 transition-all"
                            >
                                Ver mis Pedidos
                            </button>
                            <button 
